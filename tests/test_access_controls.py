...
class _ConversationManager:
    ...
    async def get_conversation(self, _umo, _cid):
        return SimpleNamespace(
            history=json.dumps([{"role": "user", "content": "private reference"}])
        )
...
async def test_cross_session_tool_is_disabled_by_default():
    result = await _plugin({}).access_others_chat_history(
...
async def test_cross_session_tool_rejects_non_admin_by_default():
    result = await _plugin({"tool_enabled": True}).access_others_chat_history(
